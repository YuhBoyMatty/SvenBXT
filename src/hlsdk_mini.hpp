#ifndef HLSDK_MINI_HPP
#define HLSDK_MINI_HPP
...
public:
	...
	inline Vector(const float rgfl[3])
	{
		x = rgfl[0];
		y = rgfl[1];
		z = rgfl[2];
...
typedef struct cl_enginefuncs_s
{
	...
	void (*SPR_Draw)(int frame, int x, int y, const rect_s* prc);
	void (*SPR_DrawHoles)(int frame, int x, int y, const rect_s* prc);
	void (*SPR_DrawAdditive)(int frame, int x, int y, const rect_s* prc);
	...
	void (*SetCrosshair)(HSPRITE_HL hspr, rect_s rc, int r, int g, int b);
	...
	int (*HookUserMsg)(const char* szMsgName, pfnUserMsgHook pfn);
	...
	const struct model_s* (*GetSpritePointer)(HSPRITE_HL HSPRITE_HL);
	...
	const struct model_s* (*PrecacheSprite)(HSPRITE_HL spr);
	...
	void (*SPR_DrawGeneric)(int frame, int x, int y, const rect_s* prc, int blendsrc, int blenddst, int unknown1, int unknown2);
